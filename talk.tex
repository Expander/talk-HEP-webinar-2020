...
\begin{frame}{Berechnung in einer Effektiven Feldtheorie}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=0.9]
      ...
      \draw[<-, thick, red] (4.1,1) -- (4.5,1) node[right]{$M_h^2 = \lambda(M_t) v^2 + (\Delta m_h^2)_{\SM}$};
      \draw[<-, thick, blue] (4.2,1.2) -- node[right, align=center]{Renormierungsgruppenlaufen,\\
...
\begin{frame}{Summation von $L^n$}
  ...
  \begin{align*}
    ...
    \right]
  \end{align*}
  $\Rightarrow$ $M_h$ enthält \emph{unendliche Reihe} von $(\kappa L)^n$-Termen
\end{frame}
