...
\begin{frame}{Example spectrum generator: MSSM}
  \begin{raggedright}
    \begin{tikzpicture}[scale=0.9]
      ...
      \draw[<-, thick, blue] (4.1,2) -- (4.5,2) node[right]{calculation of masses/observables};
      \draw[<-, thick, darkgreen] (4.1,1) -- (4.5,1) node[right]{fix $g_i^\BSM$, $y_f^\BSM$, $v^\BSM$};
    \end{tikzpicture}
  \end{raggedright}
\end{frame}
...
\begin{frame}{FlexibleSUSY resources}
  \tikz [remember picture,overlay]
  \node at
    ([yshift=-1.5cm,xshift=4.0cm]current page.north)
  ...
  \begin{center}
    ...
    \href{https://github.com/FlexibleSUSY/FlexibleSUSY}{github.com/FlexibleSUSY}
  \end{center}
\end{frame}
...
\begin{frame}{Problem: Is the predicted Higgs mass too small?}
  ...
  \begin{align*}
    m_h^2 = m_Z^2 \cos^2 2\alpha \le m_Z^2
  \end{align*}
...
\begin{frame}{Fixed-order calculation}
  ...
  \begin{align*}
    ...
    &\approx \frac{6 y_t^4 v^2}{(4\pi)^2} \left(
      ...
      + \frac{X_t^2}{\MS^2}
      - \frac{X_t^4}{12 \MS^4}
    \right) + \ord\left(\frac{v^2}{\MS^2}\right)
  \end{align*}
\end{frame}
...
\begin{frame}{Higgs mass at 2-loop level}
  Known contributions: $\ord(\as (\at + \ab) + (\at+\ab)^2 + \atau^2)$
...
\begin{frame}{Higgs mass at 2-loop level}
  \begin{align*}
    ...
    \frac{y_t^6 v^2}{(4\pi)^4} \left(
      c_1 \log^2\frac{\MS^2}{m_t^2}
      + c_2 \log\frac{\MS^2}{m_t^2}
      + c_3
    \right) \\
    & +
    \frac{y_t^4 g_3^2 v^2}{(4\pi)^4} \left(
...
\begin{frame}{Higgs mass at 3-loop level}
  Known contributions: $\ord(\at\as^2)$ for $p^2 = 0$ \mycite{1005.5709,1708.05720}
  \begin{center}
    \includegraphics[width=0.9\textwidth]{images/h3l-atasas}
  \end{center}
  \begin{align*}
    (\Delta m_h^2)^{3L} &\approx
    \frac{y_t^4 g_3^4 v^2}{(4\pi)^6} \left(
...
\begin{frame}{Summary of fixed loop order calculation}
  ...
  \begin{align*}
    ...
    &\approx [91 + \ord(20\ldots 30) + \ord(2\ldots 4) + \ord(1\ldots 2)] \GeV
  \end{align*}
  \emph{Advantages:}
  \begin{itemize}
  \item[\ok] includes logarithmic, non-logarithmic and suppressed terms of
    $\ord((v^2/\MS^2)^\infty)$ at fixed loop order
  ...
  \begin{align*}
    M_h^2 &= m_Z^2 c^2_{2\beta}
          ...
          &= m_Z^2 c^2_{2\beta}
            ...
            + \frac{736}{3} g_3^4 \textcolor{red}{\kappa^3 L^3} + \ord(\textcolor{red}{\kappa^4L^4})
            \right]
  \end{align*}
...
\begin{frame}{Summary of EFT approach}
  ...
  \begin{align*}
    ...
    &\approx [\ord(124) + \ord(0.5\ldots 1) + \ord(0.1\ldots 0.2) + \ord(0.02\ldots 0.04)] \GeV
    % &= \sqrt{\lambda(\MS)} v + \text{logs} + \Delta m_h^{1L} + \Delta m_h^{2L} + \cdots \\
    % &\approx [\ord(84) + \ord(40) + \ord(0.5\ldots 1) + \ord(0.1\ldots 0.2)] \GeV
  \end{align*}
  \emph{Advantages:}
  \begin{itemize}
  ...
  \emph{Disadvantage:}
  \begin{itemize}
  \item[\notok] Terms of $\ord(v^2/M_S^2)$ are (typically) neglected \\
