...
\begin{frame}{Eigenschaften des MSSM}
  \begin{columns}
    ...
    \item {\color<2>{darkgreen}{korrekte Vorhersage von $(g-2)_\mu$}}
...
\begin{frame}{Feste Schleifenordnung vs.\ Effektive Feldtheorie}
  \begin{center}
    \begin{tikzpicture}[scale=0.9]
      ...
      \draw[thick] (1,0)   rectangle node{SM}    (4,0.9);
      \draw[thick] (1,1.1) rectangle node{MSSM}  (4,6);
      \draw[<-, thick, blue] (4.1,5) -- (4.5,5) node[right]{$M_h$};
...
\begin{frame}{Berechnung in einer Effektiven Feldtheorie}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=0.9]
      ...
      \draw[<-, thick, blue] (4.1,5) -- (4.5,5) node[right]{$\lambda(\MS) = \frac{1}{4}\left(g_Y^{2} + g_2^2\right) c^2_{2\beta} + \cdots$};
      \draw[<-, thick, blue] (4.1,1) -- (4.5,1) node[right]{$M_h^2 = \lambda(M_t) v^2 + (\Delta m_h^2)_{\SM}$};
      \draw[<-, thick, red] (4.2,1.2) -- node[right, align=center]{Renormierungsgruppenlaufen,\\
        d.h.\ numerisches Lösen von\\
        $\frac{\dd\lambda}{\dd L} = \beta_\lambda$} (4.2,4.8);
    \end{tikzpicture}
  \end{center}
\end{frame}
