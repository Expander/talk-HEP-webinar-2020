...
\begin{frame}{Use the Higgs mass to put constraints on a SUSY model}
  ...
  \item Constrain the parameter space by requiring:
    \begin{align*}
      M_h \overset{!}{=} 125.14 \GeV \pm \Delta M_h^{\text{exp}} \pm \Delta M_h^{\text{theo}}
    \end{align*}
  \end{enumerate}
...
\begin{frame}{Fixed-order calculation}
  \emph{Input:} $\aem(M_Z)$, $\as(M_Z)$, $G_F$, $M_Z$, $M_t$, $m_b(m_b)$, SUSY parameters, \ldots\\[2em]
  \begin{center}
    \begin{tikzpicture}[scale=0.9]
