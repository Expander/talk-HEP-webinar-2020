...
\date{Physik-Kolloquium\\ Europa-Universität Flensburg\\[1em]
  24.10.2019}
...
\begin{frame}{Wechselwirkungen im Standardmodell}
  ...
  \begin{center}
    ...
    \begin{tabular}{llll}
      \toprule
      WW & Austauschteilchen & Kopplung & Symmetriegruppe\\
      \midrule
      schwache Hyperladung & $B$              & $g_Y$ & $U(1)_Y$ \\
      schwacher Isospin & $W^1, W^2, W^3$  & $g_2$ & $SU(2)_L$ \\
      starke      & $G^1, \dots, G^8$ & $g_3$ & $SU(3)_C$ \\
      \bottomrule
    \end{tabular}}
  \end{center}
  \vspace*{1em}
  ...
  \begin{center}
    ...
    \begin{tabular}{llll}
      ...
      WW & Austauschteilchen & Kopplung & Symmetriegruppe\\
      \midrule
      elektromagnetische & $\gamma$ & $e$ & $U(1)_\text{e.m.}$ \\
      schwache & $W^+$, $W^-$, $Z$ & -- & -- \\
      starke   & $G^1, \dots, G^8$ & $g_3$ & $SU(3)_C$ \\
      \bottomrule
    \end{tabular}}
...
\begin{frame}{Minimal Supersymmetrisches Standardmodell (MSSM)}
  \begin{center}
    \begin{tikzpicture}[node distance = 2.5em, auto]
        \node[quark] (u) {$u$};
        \node[quark, below of=u] (d) {$d$};
...
\begin{frame}{Eigenschaften des MSSM}
  \begin{columns}
    ...
    \emph{Vorteile:}
    \begin{itemize}
    \item {\color<1>{darkgreen}{Eichkopplungsvereinigung}}
    \item {\color<2>{darkgreen}{korrekte Vorhersage von $g_\mu$}}
    \item {\color<3>{darkgreen}{Enthält ein Dunkle Materie-Teilchen}}
      % \item (löst das Hierarchie-Problem)
    \item {\color<4>{darkgreen}{Vorhersage der Masse des Higgs-Bosons}}
    \end{itemize}
    \emph{Probleme:}
    \begin{itemize}
...
\begin{frame}{Wie kann man das MSSM testen?}
  ...
  \item Einschränkung des Parameterraums des MSSM durch Forderung:
    ...
    \end{align*}
  \end{enumerate}
  \emph{Experimentelle Unsicherheit:}
  \begin{align*}
...
\begin{frame}{Berechnung bis zu endlicher Schleifenordnung}
  ...
  \item Je mehr Schleifen wir mitnehmen, desto genauer das Ergebnis!
  \end{itemize}
  \vspace*{1em}
  \emph{Idee:}
