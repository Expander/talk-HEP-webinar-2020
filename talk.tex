...
\begin{frame}{Wechselwirkungen}
  ...
  \begin{itemize}
  \item Teilchenumwandlung (Erzeugung, Vernichtung, ``Zerfall'')
  \item Kraft zwischen Teilchen (z.B.\ Coulombkraft)
  \end{itemize}
  \vspace*{1em}
...
\subsection{Wie können wir das MSSM testen?}

\begin{frame}{Wie können wir das MSSM testen?}
  \emph{Idee:}
  \begin{enumerate}
...
\begin{frame}{Los geht's! 1-Schleifen-Quantenkorrekturen}
  Dominante Quantenkorrekturen zu $M_h$ auf 1-Schleifenniveau:
  \begin{align*}
    (\Delta m_h^2)^{1L}
...
\begin{frame}{1-Schleifen-Quantenkorrekturen}
  ...
  \emph{Beobachtungen:}
  \begin{itemize}
  \item logarithmisch verstärkt mit $\MS / m_t$
  \item maximal für $x_t \approx \sqrt{6}$
  \item extrem sensitiv auf Wert von $y_t$
  \item damit $M_h \approx 125\eh{GeV}$ muss $\MS \gtrsim 5\eh{TeV}$
  \item verbleibende Unsicherheit: $\delta M_h \approx \pm 6 \GeV$
  \end{itemize}
  Nicht gut genug!
\end{frame}

\begin{frame}{Weiter geht's! 2-Schleifen-Quantenkorrekturen}
  Bekannte Beiträge \mycite{hep-ph/0105096, hep-ph/0112177}:
  \\[1em]
...
\begin{frame}{Weiter geht's! 3-Schleifen-Quantenkorrekturen}
  Bekannte Beiträge \mycite{1005.5709}:
  \begin{center}
...
\begin{frame}{Lösung: Effektive Feldtheorie}
  ...
  \begin{align*}
    \Delta m_h^2 \ni c_1 L^1 + c_2 L^2 + c_3 L^3 + O(\textcolor{red}{L^4})
  \end{align*}
  \emph{Idee:} Verwende eine Methode, bei der \emph{alle} Terme von der Form
  \begin{align*}
    \Delta m_h^2 \ni \sum_{n=0}^\infty c_n L^n
  \end{align*}
  einbezogen werden:
  \begin{center}
...
\begin{frame}{Zusammenfassung}
  ...
  Standardmodells.
  \\[1em]
  \emph{Präzise Vorhergesagte der Masse des Higgs-Bosons} erlaubt es
  den Parameterraum des MSSM einzuschränken.
  \\[1em]
  \emph{Stopsmassen} $\MS \gtrsim 2\TeV$ nötig für korrekte Vorhersage
  $M_h = 125.09\GeV$.
  \\[1em]
  \emph{Effective Feldtheorie} ist nötig um präzise Vorhersagen zu
  erhalten, da Reihe großer Logarithmen aufsummiert
\end{frame}

\begin{frame}{Ausblick}
  \begin{center}
    \LARGE Riesiger Zoo an Modellen $\Rightarrow$ Automatisierung nötig!
  \end{center}
  \begin{center}
