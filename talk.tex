...
\begin{frame}{Further improvements: $x_t$ resummation}
  ``With large stop mixing comes large uncertainty.'' [P.\ Slavich].\\[1em]
  $\DMh$ maximal when $x_t \equiv X_t/\MS \approx \pm \sqrt{6}$:
  \begin{center}
...
\begin{frame}{Recap: $\tan\beta$ resummation in $m_b$}
  Relation between running $m_b$ in EFT and BSM model:
  \begin{align*}
...
\begin{frame}{$x_t$ resummation}
  Relation between running $y_t$ in EFT and BSM model:
  \begin{align*}
...
\begin{frame}{Summary}
  ...
  \\[1em]
  \emph{Stops must be heavy}, $\MS \gtrsim 2\TeV$, in the MSSM
  necessary for correct prediction of $M_h = 125.10\GeV$ $\rightarrow$
  \\[1em]
  \emph{Effective field theory} methods necessary for a precise
  prediction (resummation of large logarithmic corrections)
\end{frame}
...
\begin{frame}[noframenumbering]{Eigenschaften der EFT-Rechnung}
